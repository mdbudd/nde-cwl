...
export const getContentType = (url) => {
  ...
  const dom = process.env.DOM1 || "/"
  if (cleanedUrl.includes(`${dom}/news/`)) {
    contentType = "News"
  }
  if (cleanedUrl.includes(`${dom}/events/`)) {
    contentType = "Events"
  }
  if (
    cleanedUrl.includes(`${dom}/blogs/`) ||
    cleanedUrl.includes(`${dom}/blog/`)
  ) {
    contentType = "Blogs"
  }

  if (cleanedUrl.includes(`${dom}/funding/`)) {
    contentType = "Funding"
  }
  return contentType
}
